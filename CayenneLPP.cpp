...
uint8_t CayenneLPP::addGPS_highPrec(uint8_t channel, float latitude, float longitude, float meters)
{
  if ((cursor + LPP_GPS_HIGHPREC_SIZE) > maxsize)
  {
    return 0;
  }
  ...
  buffer[cursor++] = channel;
  buffer[cursor++] = LPP_GPS_HIGHPREC;
...
uint8_t CayenneLPP::addCapacityBattery(uint8_t channel, uint8_t percent)
{
  if ((cursor + LPP_CAPACITY_BATT_SIZE) > maxsize)
  {
    return 0;
  }
  buffer[cursor++] = channel;
  buffer[cursor++] = LPP_CAPACITY_BATT;
